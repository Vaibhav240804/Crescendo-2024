...
# Process reviewList to extract sentences and lemmatize them
review_sentences = [review['body'] for review in reviewList]

def lemmatize_sentence(sentence):
    lemmatized_words = [lemmatizer.lemmatize(word) for word in sentence.split() if word.lower() not in stop_words]
    return ' '.join(lemmatized_words)

lemmatized_sentences = [lemmatize_sentence(sentence) for sentence in review_sentences]

# Use CountVectorizer and NMF to extract topics
...
# Extract related sentences for each top word
related_sentences = {word: [] for word in overall_top_words}
for sentence in review_sentences:
    for word in overall_top_words:
        if word in sentence:
            related_sentences[word].append(sentence)

# Now you can define your Flask route to serve related sentences
